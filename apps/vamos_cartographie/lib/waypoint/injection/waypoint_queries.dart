...
@riverpod
GraphNode<Waypoint> waypointNode(Ref ref, TripId tripId, WaypointId id) {
...
@riverpod
NodeState<Waypoint> waypoint(Ref ref, TripId tripId, WaypointId id) {
  final node = ref.watch(waypointNodeProvider(tripId, id));
  return node.current;
}

@riverpod
List<Waypoint> allWaypoint(Ref ref, TripId tripId) {
  // Attention, ne fait pas de watch sur les StateNode<Vertex>
  final store = ref.watch(waypointStoreProvider(tripId));
  return store.waypointStore.store.values
      .map((v) => v.current.displayValue)
      .toList();
}
