...
class CreateSegment {
  ...
  Future<void> call(
    ...
    await executor.run(
      onApply: () {
        tempId = store.segmentStore.create(
          (id) => draft.toSegment(id, optimistGeometry),
        );
      },
      remote: () => port.createSegment(graphId, draft),
      onSuccess: (segment) {
        store.segmentStore.commitCreate(tempId: tempId, serverEntity: segment);
      },
      onError: () {
        store.segmentStore.rollbackCreate(tempId);
      },
    );
  }
}
